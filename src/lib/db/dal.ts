// CareLink — Typed Data Access Layer (DAL)
// Column names match /supabase/schema.sql exactly.
// All functions throw on error; callers wrap with try/catch.
...
export const pros = {
  ...
  async upsert(p: Partial<Professional> & { id: UUID; specialty: ProSpecialty }): Promise<Professional> {
    return unwrap(
      await supabase
        .from("professionals")
  ...
  /** List from v_pros_public view (verified + available pros) */
...
export const bookings = {
  ...
  /** Accept a bid: mark booking as matched, set professional_id */
  async acceptBid(bookingId: UUID, proId: UUID, finalPrice: number): Promise<Booking> {
    return unwrap(
      await supabase
        .from("bookings")
        .update({
...
export const bids = {
  ...
  /** Accept this bid and reject all others for the same booking */
...
export const ratings = {
  async create(
    input: Omit<Rating, "id" | "created_at">
  ): Promise<Rating> {
    return unwrap(
