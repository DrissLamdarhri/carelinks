...
// });
/**
 * CareLink — BookingMap (upgraded)
 * ──────────────────────────────────────────────────────────────────────────────
 * Drop-in replacement for the old BookingMap.
 * Uses CareLinkMap (100% free, no API key, works iOS + Android).
 *
 * Features:
 *   • Custom branded SVG map (CareLink palette)
 *   • Draggable pin via GPS or tap-to-place
 *   • "My location" button (expo-location)
 *   • Radius indicator
    ...
    const res = await fetch(
      `https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lng}&accept-language=fr`,
      {
        headers: {
          ...
          "User-Agent": "CareLink-App/1.0 (contact@carelink.ma)",
        },
      }
    );
...
const DEFAULT_LAT = 34.0372;
const DEFAULT_LNG = -5.0039;
...
export function BookingMap({
  ...
  // Geocode after coordinate changes (debounced 700ms)
  const scheduleGeocode = useCallback((lat: number, lng: number) => {
    if (geocodeTimer.current) clearTimeout(geocodeTimer.current);
    geocodeTimer.current = setTimeout(async () => {
      // Try expo-location first (faster, offline)
      const local = await geo.reverseGeocode(lat, lng);
      if (local) {
        setAddress(local);
        return;
      }
      // Fallback to Nominatim (online, 100% free)
      const nominatim = await reverseGeocodeNominatim(lat, lng);
      setAddress(nominatim);
    }, 700);
  }, []);
  ...
  }, [initialLat, initialLng, scheduleGeocode]);

  const handleMapPress = useCallback(
    (coord: LatLng) => {
      setMarkerCoord(coord);
      onChange?.(coord.lat, coord.lng);
      scheduleGeocode(coord.lat, coord.lng);
    },
    [onChange, scheduleGeocode]
  );
  ...
  }, [locating, onChange, scheduleGeocode]);

  return (
    <View style={[styles.wrap, { borderRadius: 20, overflow: "hidden" }]}>
      {/* ── Map ──────────────────────────────────────────────────────────── */}
      <CareLinkMap
      ...
      {/* ── GPS button ───────────────────────────────────────────────────── */}
      <TouchableOpacity
        style={[styles.gpsBtn, { borderColor: `${primaryColor}22` }]}
      ...
      >
        {locating ? (
          ...
          <LocateFixed size={14} color={primaryColor} />
        )}
        <Text style={[styles.gpsBtnText, { color: primaryColor }]}>
          {locating ? "Localisation…" : "Ma position"}
        </Text>
      ...
      <View style={styles.footer}>
        ...
        <Text style={styles.footerText} numberOfLines={1}>
          {address
            ? address
            : `${markerCoord.lat.toFixed(4)}, ${markerCoord.lng.toFixed(4)}`}
        </Text>
        <View style={[styles.radiusPill, { borderColor: `${primaryColor}30` }]}>
          <Text style={[styles.radiusText, { color: primaryColor }]}>
            {radiusKm} km
          </Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  radiusPill: {
    ...
    borderRadius: 10,
    borderWidth: 1,
    backgroundColor: "rgba(13,8,112,0.05)",
  },
  radiusText: {
    fontSize: 11,
    fontWeight: "600",
